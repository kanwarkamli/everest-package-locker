...
import { CryptoCodeGenerator, SequentialCodeGenerator } from '../application/code-generator.js';
import { InMemoryLockerRepository } from '../infrastructure/in-memory-locker-repository.js';
import { runRepl } from './repl.js';

// LOCKER_DEMO=1 makes pickup codes deterministic so scripted demos/recordings
// can retrieve packages; real runs always use crypto-random codes.
const demoMode = process.env.LOCKER_DEMO === '1';

const station = new LockerStation({
  repository: new InMemoryLockerRepository(),
  strategy: new SmallestFitStrategy(),
  clock: new SystemClock(),
  codeGenerator: demoMode ? new SequentialCodeGenerator() : new CryptoCodeGenerator(),
